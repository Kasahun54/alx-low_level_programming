...
/**
 * print_data - Print the ELF header data
...
/**
 * print_version - Print the version the ELF header
 * @e_ident: pointer to an array contain the ELF version
 */
void print_version(unsigned char *e_ident)
{
 ...
 *
 * Description: If the file can't be exit - exit code is 98
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
